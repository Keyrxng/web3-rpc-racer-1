...
export class RPCHandler implements HandlerInterface {
  ...
  public async getFastestRpcProvider(): Promise<JsonRpcProvider> {
    ...
    if (fastest && fastest?.connection.url.includes("localhost") && this._networkId !== 31337) {
      /**
  ...
  public async testRpcPerformance(): Promise<JsonRpcProvider> {
    ...
    this._provider = this.createProviderProxy(new JsonRpcProvider(fastestRpcUrl, this._networkId), this);
